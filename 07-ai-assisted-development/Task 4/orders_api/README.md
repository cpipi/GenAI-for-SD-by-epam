...
### Pagination & filtering behavior (implementation details)

- **Pagination defaults**:
  - `page` is 1-based and defaults to `1` when omitted.
  - `limit` defaults to `10` and is capped at a maximum of `100`.
- **Offset calculation**:
  - The server uses `offset = (page - 1) * limit` to fetch the correct slice of orders.
  - When you request a page beyond the available data, the API returns an empty `items` array with valid `page`, `limit`, `total`, and `total_pages` metadata.
- **Server-side filtering**:
  - All filters (`status`, `min_amount`, `max_amount`, `start_date`, `end_date`) are applied in the database query (no in-memory filtering).
  - `start_date`/`end_date` filter by the `created_at` field.
- **Validation & error handling**:
  - If `start_date` is after `end_date`, the API returns `400 Bad Request` with a descriptive error.
  - If `min_amount` is greater than `max_amount`, the API returns `400 Bad Request`.
  - Invalid query parameter types or out-of-range values (e.g., `limit > 100`) are rejected with `422 Unprocessable Entity` by FastAPI/Pydantic validation.
